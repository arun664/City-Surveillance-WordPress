...
<div class="top_panel_navi sc_layouts_row sc_layouts_row_type_compact sc_layouts_row_fixed sc_layouts_row_fixed_always sc_layouts_row_delimiter<?php
			if (false) {
			echo ' scheme_'. esc_attr(prolingua_is_inherit(prolingua_get_theme_option('menu_scheme')) 
		...
		<div class="columns_wrap columns_fluid">
			...
			?><div class="sc_layouts_column sc_layouts_column_align_right sc_layouts_column_icons_position_left sc_layouts_column_fluid column-3_4">
				<div class="sc_layouts_item">
					<?php
					// Main menu
					$prolingua_menu_main = prolingua_get_nav_menu(array(
						'location' => 'menu_main', 
						'class' => 'sc_layouts_menu sc_layouts_menu_default sc_layouts_hide_on_mobile'
						)
					);
					// Show any menu if no menu selected in the location 'menu_main'
					if (prolingua_get_theme_setting('autoselect_menu') && empty($prolingua_menu_main)) {
						$prolingua_menu_main = prolingua_get_nav_menu(array(
							'class' => 'sc_layouts_menu sc_layouts_menu_default sc_layouts_hide_on_mobile'
							)
						);
					}
					prolingua_show_layout($prolingua_menu_main);
					// Mobile menu button
					?>
				...
				if (prolingua_exists_trx_addons()) {
					?><div class="sc_layouts_item"><?php
						...
						do_action('prolingua_action_search', 'fullscreen', 'header_search', false);
					?></div><?php
				}
				?>
			</div>
