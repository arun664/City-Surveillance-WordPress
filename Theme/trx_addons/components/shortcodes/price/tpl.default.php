		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_price_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="'.esc_attr__('img', 'prolingua').'"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
				...
				if (!empty($icon)) {
			        ...
			        ?><div class="sc_price_item_plan_wrapper"><?php
				...
				if (!empty($item['link'])) {
					?><a href="<?php echo esc_url($item['link']); ?>" class="sc_price_item_link sc_button<?php echo (isset($item['link_text']) && $item['link_text'] ? ' with_text_link' : ' without_text_link')?>"><?php
