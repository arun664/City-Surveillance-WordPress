	...
	if (!empty($address) || !empty($phone) || !empty($email)) {
		?><div class="contacts_info"><?php
			if (!empty($address)) {
				...
				?><span class="address_block"><span class="address_label"><?php echo esc_html__('Address','prolingua')?></span><span class="contacts_address"><?php echo str_replace('|', "<br>", $address); ?></span></span><?php
				if ($columns) {
					?></div><?php
				}
			}
			if (!empty($phone) || !empty($email)) {
				...
				if (!empty($email)) {
                    ?><span class="email_block"><span class="email_label"><?php echo esc_html__('Email','prolingua')?></span><span class="contacts_email"><a href="mailto:<?php echo antispambot($email); ?>"><?php echo antispambot($email); ?></a></span></span><?php
				}
				if (!empty($phone)) {
					?><span class="phone_block"><span class="phone_label"><?php echo esc_html__('Phone','prolingua')?></span><a href="<?php echo esc_attr(trx_addons_get_phone_link($phone)); ?>" class="contacts_phone"><?php echo wp_kses_data($phone); ?></a></span><?php
				}
				if ($columns) {
					?></div><?php
				}
