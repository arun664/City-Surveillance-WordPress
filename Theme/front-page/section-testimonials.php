<div class="front_page_section front_page_section_testimonials<?php
			$prolingua_scheme = prolingua_get_theme_option('front_page_testimonials_scheme');
			if (!prolingua_is_inherit($prolingua_scheme)) echo ' scheme_'.esc_attr($prolingua_scheme);
			echo ' front_page_section_paddings_'.esc_attr(prolingua_get_theme_option('front_page_testimonials_paddings'));
		?>"<?php
		$prolingua_css = '';
		$prolingua_bg_image = prolingua_get_theme_option('front_page_testimonials_bg_image');
...
?>><?php
	// Add anchor
	$prolingua_anchor_icon = prolingua_get_theme_option('front_page_testimonials_anchor_icon');	
	$prolingua_anchor_text = prolingua_get_theme_option('front_page_testimonials_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_testimonials_inner<?php
			if (prolingua_get_theme_option('front_page_testimonials_fullheight'))
				echo ' prolingua-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$prolingua_css = '';
			$prolingua_bg_mask = prolingua_get_theme_option('front_page_testimonials_bg_mask');
			$prolingua_bg_color = prolingua_get_theme_option('front_page_testimonials_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$prolingua_caption = prolingua_get_theme_option('front_page_testimonials_caption');
			if (!empty($prolingua_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo !empty($prolingua_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post($prolingua_caption); ?></h2><?php
			}
		
			// Description (text)
			$prolingua_description = prolingua_get_theme_option('front_page_testimonials_description');
			if (!empty($prolingua_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo !empty($prolingua_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post(wpautop($prolingua_description)); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_testimonials_output"><?php 
				if (is_active_sidebar('front_page_testimonials_widgets')) {
					...
					if (!prolingua_exists_trx_addons())
						...
						prolingua_customizer_need_widgets_message('front_page_testimonials_caption', 'ThemeREX Addons - Testimonials');
				}
			?></div>
		</div>
	</div>
