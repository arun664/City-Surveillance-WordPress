...
if ( !function_exists('prolingua_need_page_title') ) {
	function prolingua_need_page_title() {
		return !is_front_page() && apply_filters('prolingua_filter_need_page_title', true);
	}
}
...
if ( !function_exists('prolingua_show_layout') ) {
	function prolingua_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('prolingua_get_logo_image') ) {
	function prolingua_get_logo_image($type='') {
		$logo_image = '';
		if (prolingua_is_on(prolingua_get_theme_option('logo_retina_enabled')) && prolingua_get_retina_multiplier(2) > 1)
			$logo_image = prolingua_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) {
			if (empty($type) && function_exists('the_custom_logo')) {
				...
				$logo_image = prolingua_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
			}
		}
		return prolingua_remove_protocol_from_url($logo_image, false);
	}
}

// Return header video (if set)
if ( !function_exists('prolingua_get_header_video') ) {
	function prolingua_get_header_video() {
		$video = '';
		if (apply_filters('prolingua_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (prolingua_check_theme_option('header_video')) {
				$video = prolingua_get_theme_option('header_video');
...
if ( !function_exists('prolingua_show_post_meta') ) {
	function prolingua_show_post_meta($args=array()) {

		if (is_single() && prolingua_is_off(prolingua_get_theme_option('show_post_meta'))) return ' ';	// Space is need!
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post categories
				if ($comp == 'categories') {
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('prolingua_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><?php prolingua_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('prolingua_filter_get_post_date', prolingua_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post author
					...
					if ($author_id > 0) {
						...
						<span class="author_label"><?php echo esc_html('by');?></span>
				...
				// Post counters
					...
					if ( ($output = prolingua_get_post_counters($args['counters'])) != '')
						prolingua_show_layout($output);
					else {
						if (!is_singular() || have_comments() || comments_open()) {
							...
							<a href="<?php echo esc_url(get_comments_link()); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><?php
								...
								?><span class="post_counters_label"><?php
									echo esc_html(_n('Comment', 'Comments', $post_comments, 'prolingua'));
								?></span>
							</a>
							<?php
						}
				...
				// Socials share
					...
					prolingua_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'prolingua'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'prolingua' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('prolingua_show_post_featured') ) {
	function prolingua_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => prolingua_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => prolingua_is_on(prolingua_get_theme_option('seo_snippets')),
		...
		$thumb_size = !empty($args['thumb_size']) 
						? $args['thumb_size'] 
						: prolingua_get_thumb_size(is_attachment() || is_single() ? 'full' : 'big');
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? prolingua_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = prolingua_get_post_image();
				if (!empty($image)) 
					$image = prolingua_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . prolingua_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>">
					...
					if (!$args['thumb_bg']) 
						echo wp_get_attachment_image( get_the_ID(), $thumb_size, false, 
													 prolingua_is_on(prolingua_get_theme_option('seo_snippets'))
				...
				?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$prolingua_attr = prolingua_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($prolingua_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($prolingua_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						prolingua_hovers_add_icons($args['hover']);
					}
					...
					if (($output=prolingua_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						prolingua_show_layout($output);
					...
					if (!empty($image)) {
						...
						prolingua_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = prolingua_get_post_video($post_content, false);
						if (empty($video))
							$video = prolingua_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = prolingua_array_get_first($videos, false);
								}
							}
						}
						if (!empty($video)) {
							if ( $has_thumb ) {
								$video = prolingua_make_video_autoplay($video);
								?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
							}
							?><div class="post_video video_frame"><?php 
								if ( !$has_thumb ) {
									prolingua_show_layout($video);
								}
							?></div><?php
						}
					...
					// Put audio over the thumb
						...
						$audio = prolingua_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = prolingua_get_post_iframe($post_content, false);
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								// Add author and title
								$media_author = prolingua_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = prolingua_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !prolingua_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php prolingua_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !prolingua_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php prolingua_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								prolingua_show_layout($audio); 
							?></div><?php
						}
					}
				}
				...
				prolingua_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				prolingua_show_layout($post_info);
...
if ( !function_exists('prolingua_get_no_image') ) {
	function prolingua_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = prolingua_get_theme_option( 'no_image' );
			if (empty($img)) $img = prolingua_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('prolingua_add_bg_in_post_nav') ) {
	function prolingua_add_bg_in_post_nav() {
		if ( ! is_single() ) return;

		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = prolingua_get_no_image();

		if ( is_attachment() && $previous->post_type == 'attachment' ) return;

		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), prolingua_get_thumb_size('med') );
				$img = $img[0];
			}
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), prolingua_get_thumb_size('med') );
				$img = $img[0];
			}
		...
		wp_add_inline_style( 'prolingua-main', $css );
	}
}
...
// Show related posts
if ( !function_exists('prolingua_show_related_posts') ) {
	function prolingua_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : prolingua_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'prolingua');
				?></h3>
...
if ( !function_exists('prolingua_show_portfolio_posts') ) {
	function prolingua_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? prolingua_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = prolingua_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = prolingua_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the prolingua_ajax_get_posts action
if ( !function_exists( 'prolingua_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_prolingua_ajax_get_posts',			'prolingua_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_prolingua_ajax_get_posts',	'prolingua_ajax_get_posts_callback');
	function prolingua_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			prolingua_storage_set('blog_mode', 'blog');
			prolingua_storage_set('options_meta', get_post_meta($id, 'prolingua_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => prolingua_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'prolingua');
		}
...
// Show pagination
if ( !function_exists('prolingua_show_pagination') ) {
	function prolingua_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = prolingua_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( 'Prev', 'prolingua' ),
				'next_text' => esc_html__( 'Next', 'prolingua' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'prolingua' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'prolingua'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'prolingua') : esc_html__('Newest posts', 'prolingua') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'prolingua') : esc_html__('Older posts', 'prolingua'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
